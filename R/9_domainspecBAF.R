...
dataMedian <- dplyr::summarise(group_by(plotdf, Factor, Domain), 
...
selectnocolor %>%
  ...
  geom_text(data = dataMedian, aes(Domain, Pos, label = paste("m =", MD), color = Domain), 
            size = 3, vjust = -3, hjust= -0.05, fontface = "bold", angle=0) +
  ...
  theme(strip.background = element_rect(fill="grey99", color="grey", size =1), 
        ...
        panel.border =element_rect(colour="grey", size = 0.2), 
