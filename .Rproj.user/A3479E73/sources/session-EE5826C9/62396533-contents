...
#### DO ANY CUSTOM FILTERING YOU WOULD LIKE HERE ####

# filter only samples with metadata available for SLR
metadatacom <- metadatacom %>%
  select(SampleSiteLocationDescription, 
         TempdegC, 
         Depthm) %>%
  drop_na()
commonsamp <- rownames(metadatacom)
...
# If you've done a custom list (e.g. split physeq)
# then remove the comment from following line
# physeqlist <- physeqlistcust
